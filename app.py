...
def save_to_excel(product_list, file_name='products.xlsx'):
    """
    Speichert die Produktliste in einer Excel-Datei.
    """
    df = pd.DataFrame(product_list, columns=['Product Code', 'Article', 'Owner'])
    df.to_excel(file_name, index=False)

def add_product_and_save(article, owner):
    """
    Fügt ein Produkt hinzu und speichert es in Excel.
    """
    product_code = generate_product_code(article, owner)
    st.session_state.inventory_list.append({'Product Code': product_code, 'Article': article, 'Owner': owner})
    st.write(f'You added the product with the following product code: {product_code}')

    # Speichern der aktuellen Produktliste in Excel
    save_to_excel(st.session_state.inventory_list)

# ... Ihr bisheriger Code ...

# Ändern des Confirm-Buttons, um die neue Funktion aufzurufen
if confirm_button:
    add_product_and_save(article, owner)

# ... Ihr bisheriger Code ...

# Laden der Produkte aus der Excel-Datei beim Start der App
if 'inventory_list' not in st.session_state:
    try:
        st.session_state.inventory_list = pd.read_excel('products.xlsx').to_dict('records')
